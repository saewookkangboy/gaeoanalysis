...
export const SEO_GUIDELINES: Record<string, ContentGuideline> = {
  ...
  naver_blog: {
    category: 'AI SEO',
    title: '네이버 블로그 SEO (2025년 최신)',
    description: '네이버 블로그에 최적화된 SEO 가이드라인 (2025년 최신 기술 반영)',
    keyPoints: [
      '네이버 검색 알고리즘 최적화',
      '네이버 카페/밴드 공유 최적화',
      'Open Graph 태그 필수 설정',
    ...
    bestPractices: [
      'Open Graph 태그 필수 추가 (og:title, og:description, og:image, og:url)',
      '태그 3-5개 추가하여 네이버 블로그 검색 노출 증가',
      '카테고리 설정으로 블로그 구조화',
      '이미지 3개 이상 추가 (네이버는 이미지 중심 콘텐츠 선호)',
      '목차(Table of Contents) 추가로 긴 콘텐츠 가독성 향상',
      '관련글 링크 추가로 콘텐츠 간 연결성 강화',
      '댓글 기능 활성화로 사용자 참여 유도',
      'FAQPage 스키마 추가 (2025년 필수, 가장 높은 AI 인용 확률)',
      'H2→H3→bullets 구조 사용 (Perplexity에서 40% 더 많은 인용)',
      '30일 이내 콘텐츠 업데이트 (Perplexity에서 3.2배 인용 증가)',
      '통계 데이터와 인용 포함 (+41% 통계, +28% 인용 개선)',
      '구조화된 데이터(JSON-LD) 필수 포함',
    ],
    commonMistakes: [
      'Open Graph 태그 누락 (카페/밴드 공유 시 미리보기 미표시)',
      '태그 미설정 (네이버 블로그 검색 노출 감소)',
      '카테고리 미설정 (블로그 구조화 부족)',
      '이미지 부족 (네이버는 이미지 중심 콘텐츠 선호)',
      '목차 미추가 (긴 콘텐츠 가독성 저하)',
      '관련글 링크 미추가 (콘텐츠 간 연결성 부족)',
      'FAQPage 스키마 미추가 (2025년 필수 요소)',
      'H2→H3→bullets 구조 미사용 (AI 인용 확률 감소)',
      '구조화된 데이터 미포함 (AI 검색 엔진 이해도 저하)',
    ],
    implementationSteps: [
      '1. Open Graph 태그 추가 (og:title, og:description, og:image, og:url)',
      '2. 태그 3-5개 추가 (콘텐츠의 주요 키워드와 관련어)',
      '3. 카테고리 설정 (블로그 구조화)',
      '4. 이미지 3개 이상 추가 (네이버 이미지 중심 콘텐츠 선호)',
      '5. 목차(Table of Contents) 추가 (1000자 이상 콘텐츠)',
      '6. 관련글 링크 추가 (콘텐츠 간 연결성)',
      '7. 댓글 기능 활성화 (사용자 참여)',
      '8. FAQPage 스키마 추가 (2025년 필수, 가장 높은 AI 인용 확률)',
      '9. H2→H3→bullets 구조 적용 (40% 더 많은 인용)',
      '10. 30일 이내 콘텐츠 업데이트 (3.2배 인용 증가)',
      '11. 통계 데이터와 인용 포함 (+41% 통계, +28% 인용 개선)',
      '12. 구조화된 데이터(JSON-LD) 필수 포함',
    ],
  },
};
